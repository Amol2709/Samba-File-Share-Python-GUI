...
"font: 75 11pt \"Times New Roman\";")
        ...
        self.lineEdit.textChanged.connect(self.Dynamic)
...
#"background-color: rgb(85, 170, 255);")
    ...
    def retranslateUi(self, Form):
        ...
        Form.setWindowTitle(_translate("Form", "Modify SambaShare"))
    ...
    def Dynamic(self):
        ...
        #Call Parth Function and give him input Share name and Take True and List[5] if share name exist 
        # Take False if share name do not exist and Empty List



        #self.pushButton_2.setEnabled(bool(self.lineEdit.text()) and True)#len(DL)==7)
        # Test Case:
        #DL = ['amol','anypath','yes','yes','yes','yes',[]]
        # DL = ['share1','anypath','no','yes','yes','yes',[]]
        if(cf.fileexists()):
            DL = gs.getshare(self.lineEdit.text())
        else:
        ...
        if (bool(self.lineEdit.text()) and ((len(DL)==7) or (len(DL) == 6))):
            self.pushButton_2.setEnabled(True)#len(DL)==7)
            self.labeldynamic.setText(_translate("Form","<html><head/><body><p><span style=\" color:#0000ff;\">ShareName Exist</span></p></body></html>"))
            if (DL[5].lower()=='yes'):
    ...
    def SendBackend(self):
        ...
        if(not csh.exists(self.lineEdit.text())):
        	QMessageBox.warning(self.obc, "Warning","Share does not exist. Please try again with a valid name.")
        ...
        if 'None' in self.User:
            ...
            if self.radioButton_13.isChecked():
                self.U = []
                if(cf.fileexists()):
                    if(ms.modify(self.L[0],self.L[1],self.L[2],self.L[3],self.L[4],self.U)):
                        QMessageBox.information(self.obc, "Status","Share modified successfully.")
                        self.obc.close()
                else:
                    QMessageBox.warning(self.obc, "Warning","Share Modification Failed. Try modifying manually from smb.conf file")
            else:
                if(cf.fileexists()):
                    if(ms.modify(self.L[0],self.L[1],self.L[2],self.L[3],self.L[4],self.U)):
                        QMessageBox.information(self.obc, "Status","Share modified successfully.")
